...
1. 主要问题是独显无法驱动，因为官方暂未发布驱动。而且禁用独显配置不可用。虽然不影响进入系统使用，但是睡眠是有问题的，唤醒后可能会死机；
...
1. revert: 回退近期休眠相关的改动，因为并没有效果，还可能导致一些问题。
2. 由于亚历山大老哥的新版驱动有点不太给力，触摸板的VoodooI2C驱动换用了小米pro的黑果维护者@stevezhang 修改编译的临时方案。据说这个版本可以做到插鼠标的时候禁用触摸板(在辅助里开启)，不过我测试并未生效。此外发现辅助里还有个启动拖移的勾，选了就有双击按住拖拽的功能了，这就跟Windows一样咯（我喜欢这个功能，之前都不知道）。
3. 增加Lilu去除无用输出的10.13的补丁，内核崩溃时方便查看日志。
